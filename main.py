...
class App:
    ...
    def __init__(self, root):
        ...
        self.upload_button = tk.Button(root, text="Загрузить изображение", command=self.upload_image)  # создаем кнопку для загрузки изображения
        self.upload_button.pack(pady=10)  # размещаем кнопку с отступом pady=10

        # кнопка для конвертации изображения
        self.convert_button = tk.Button(root, text="Конвертировать", command=self.convert_image)  # создаем кнопку для конвертации изображения
        ...
        self.radio_pdf = tk.Radiobutton(root, text="PDF", variable=self.format_var, value="pdf") # радиокнопка для выбора PDF
        self.radio_pdf.pack()   # размещаем радиокнопку
        self.radio_word = tk.Radiobutton(root, text="Word", variable=self.format_var, value="word") # радиокнопка для выбора word
        self.radio_word.pack()  # размещаем радиокнопку

    def upload_image(self):
        """Диалоговое окно для выбора файла изображения"""
        self.file_path = filedialog.askopenfilename(filetypes=[("Image files", "*.jpg *.jpeg *.png *.pdf")])  # открываем диалоговое окно для выбора изображения
        if self.file_path:
            messagebox.showinfo("Файл выбран", os.path.basename(self.file_path))  # показываем информационное сообщение с именем выбранного файла
    ...
    def convert_image(self):
        ...
        messagebox.showinfo("Отлично!", "Конвертация завершена")  # показываем информационное сообщение о завершении операции

    def detect_language(self, file_path):
        """Автоматическое определение зыка"""
        try:
            # извлекаем текст с изображения
            temp_text = self.extract_text(file_path, lang='eng+rus+chi_sim')  # извлекаем текст с изображения с языковыми параметрами
    ...
    def save_as_pdf(self, text):
        ...
        output_path = filedialog.asksaveasfilename(defaultextension=".pdf", filetypes=[("PDF files", "*.pdf")]) # filedialog открывает диалоговое окно файла, asksaveasfilename спрашивает как назвать файл при сохранении и добавляем путь куда сохранить
    ...
    def save_as_word(self, text):
        ...
        output_path = filedialog.asksaveasfilename(defaultextension=".docx", filetypes=[("Word files", "*.docx")]) # filedialog открывает диалоговое окно файла, asksaveasfilename спрашивает как назвать файл при сохранении, так же вводим путь для сохр файл
        if output_path:  # если указан путь
